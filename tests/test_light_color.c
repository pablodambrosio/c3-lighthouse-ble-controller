...
int main(void)
{
    ...
    for (unsigned i = 0; i < sizeof(references) / sizeof(references[0]); ++i) {
        ...
        light_color_t color = light_color_from_rgb(rgb.r, rgb.g, rgb.b);
        CHECK(fabsf(color.x - references[i].x) < 0.00005f);
        CHECK(fabsf(color.y - references[i].y) < 0.00005f);
        CHECK(fabsf(color.brightness - references[i].luminance) < 0.00001f);
        CHECK(light_color_to_pwm(color, &pwm) == ESP_OK);
    ...
    for (unsigned i = 0; i < sizeof(greys) / sizeof(greys[0]); ++i) {
        ...
        CHECK(light_color_to_pwm(light_color_from_rgb(v, v, v), &pwm) == ESP_OK);
    ...
    for (int r = 0; r <= 255; r += 17) {
        for (int g = 0; g <= 255; g += 17) {
            for (int b = 0; b <= 255; b += 17) {
                ...
                CHECK(light_color_to_pwm(light_color_from_pwm(rgb), &pwm) == ESP_OK);
                CHECK(matches(rgb, pwm));
            }
        }
    }
    ...
    CHECK(light_color_to_pwm(light_color_from_pwm(startup), &pwm) == ESP_OK);
    CHECK(matches(startup, pwm));

    light_color_t white = BIG_LIGHT_WHITE;
    CHECK(light_color_to_pwm(white, &pwm) == ESP_OK);
    CHECK(matches(pwm, ((light_rgb_t){255, 255, 255})));
    white.brightness = 0.5f;
    CHECK(light_color_to_pwm(white, &pwm) == ESP_OK);
    CHECK(pwm.r >= 127 && pwm.r <= 128 && pwm.g >= 127 && pwm.g <= 128 &&
          pwm.b >= 127 && pwm.b <= 128);
    white.brightness = 0.0f;
    CHECK(light_color_to_pwm(white, &pwm) == ESP_OK);
    CHECK(matches(pwm, ((light_rgb_t){0, 0, 0})));
    light_color_t black = light_color_from_rgb(0, 0, 0);
    CHECK(black.y > 0.0f && black.brightness == 0.0f);

    light_color_t limited = light_color_from_pwm((light_rgb_t){255, 128, 64});
    limited.brightness = 1.0f;
    CHECK(light_color_to_pwm(limited, &pwm) == ESP_OK);
    CHECK(matches(pwm, ((light_rgb_t){255, 128, 64})));

    const light_color_t invalid[] = {
    ...
    for (unsigned i = 0; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        ...
        CHECK(light_color_to_pwm(invalid[i], &pwm) == ESP_ERR_INVALID_ARG);
        CHECK(matches(pwm, ((light_rgb_t){12, 34, 56})));
    }
    CHECK(light_color_to_pwm(BIG_LIGHT_WHITE, NULL) == ESP_ERR_INVALID_ARG);
    return 0;
}
