...
const light_color_t BIG_LIGHT_WHITE = {
    .x = 0.3127f, .y = 0.3290f, .brightness = 1.0f,
};

static light_color_t from_linear(float r, float g, float b)
{
    float xyz[3];
    for (int i = 0; i < 3; ++i) {
    ...
    if (total == 0.0f) {
        ...
        light_color_t black = BIG_LIGHT_WHITE;
        black.brightness = 0.0f;
        return black;
    }
    return (light_color_t) {
        .x = xyz[0] / total,
        .y = xyz[1] / total,
        .brightness = fminf(xyz[1], 1.0f),
    };
}
...
esp_err_t light_color_to_pwm(light_color_t color, light_rgb_t *pwm)
{
    if (pwm == NULL || !isfinite(color.x) || !isfinite(color.y) ||
        !isfinite(color.brightness) || color.x < 0.0f || color.y <= 0.0f ||
        color.x + color.y > 1.0f || color.brightness < 0.0f || color.brightness > 1.0f) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        rgb[i] = fmaxf(channel, 0.0f) * color.brightness;
    }
